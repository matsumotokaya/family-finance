...
export default function MiraiDashboard({ transactions, cardStatements }: Props) {
  ...
  const activeCardTotal = useMemo(
    () => cardItems.filter(t => !excludedIds.has(t.id)).reduce((s, t) => s + t.amount, 0),
    [cardItems, excludedIds]
  );
  const excludedTotal = useMemo(
    ...
    [bankItems, cardItems, excludedIds]
  );

  const grandTotal = activeBankTotal + activeCardTotal;
      ...
      <PageHeader isAlert={isOver} noPadBottom>
        <div className="flex items-center justify-between pt-1 pb-3">
          <div>
            ...
            <h1 className="text-white font-bold text-base leading-tight">松本未来</h1>
          </div>
          <HamburgerMenu />
        </div>
      ...
      <div className="px-4 py-4 space-y-4 max-w-lg mx-auto pb-10">
        {loading ? (
          ...
          <>
            ...
            />
            <ItemSection
              title={`VIEWカード（未来）${sel.label}`}
              activeTotal={activeCardTotal}
              items={cardItems}
              {...sectionProps}
            />
          </>
